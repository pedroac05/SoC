library IEEE;
use IEEE.STD_LOGIC_1164.ALL;
use IEEE.NUMERIC_STD.ALL;

entity dec3to8_tb is
end entity dec3to8_tb;

architecture Behavioral of dec3to8_tb is
    component dec3to8 is
        port (
            en    : in  std_logic;
            in_v  : in  std_logic_vector(2 downto 0);
            bcode : out std_logic_vector(7 downto 0)
        );
    end component;

    signal en_tb    : std_logic := '0';
    signal in_v_tb  : std_logic_vector(2 downto 0) := "000";
    signal bcode_tb : std_logic_vector(7 downto 0);

    constant T_STEP : time := 20 ns;
begin
    DUT: dec3to8
        port map (
            en    => en_tb,
            in_v  => in_v_tb,
            bcode => bcode_tb
        );

    stim_proc: process
        variable expected_val : std_logic_vector(7 downto 0);
    begin
        -- Caso 1: Enable desactivado (en = '0') -> bcode debe ser "00000000"
        en_tb <= '0';
        for i in 0 to 7 loop
            in_v_tb <= std_logic_vector(to_unsigned(i, 3));
            wait for T_STEP;
            assert bcode_tb = "00000000"
                report "ERROR en en=0: Se esperaba bcode=00000000 para in=" & integer'image(i)
                severity error;
        end loop;

        -- Caso 2: Enable activado (en = '1') -> decodificación One-Hot de 8 salidas
        en_tb <= '1';
        for i in 0 to 7 loop
            in_v_tb <= std_logic_vector(to_unsigned(i, 3));
            wait for T_STEP;
            
            expected_val := (others => '0');
            expected_val(i) := '1';

            assert bcode_tb = expected_val
                report "ERROR en en=1: Valor incorrecto para in=" & integer'image(i)
                severity error;
        end loop;

        report ">>> Simulacion de dec3to8_tb finalizada exitosamente sin errores. <<<" severity note;
        wait;
    end process;
end architecture Behavioral;
